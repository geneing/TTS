...
from TTS.ranger.ranger import Ranger
...
def train(model, criterion, criterion_st, optimizer, optimizer_st, scheduler,
    ...
    for num_iter, data in enumerate(data_loader):
        ...
        if global_step % c.print_step == 0:
            print(
                ...
                "DecoderLoss:{:.5f}  StopLoss:{:.5f} GSTLoss:{:.5f} AlignScore:{:.4f}  GradNorm:{:.5f}  "
                "GradNormST:{:.5f}  AvgTextLen:{:.1f}  AvgSpecLen:{:.1f}  StepTime:{:.2f}  "
                "LoaderTime:{:.2f}  LR:{:.6f}".format(
                    num_iter, batch_n_iter, global_step, loss.item(),postnet_loss.item(),
                    decoder_loss.item(), stop_loss.item(), gst_loss.item(), align_score,
    ...
    print("   | > EPOCH END -- GlobalStep:{} AvgTotalLoss:{:.5f} "
          ...
          "AvgStepTime:{:.2f}  AvgLoaderTime:{:.2f}".format(
              global_step, keep_avg['avg_total_loss'], keep_avg['avg_postnet_loss'],
              keep_avg['avg_decoder_loss'], keep_avg['avg_gst_loss'], keep_avg['avg_stop_loss'],
...
def main(args):  # pylint: disable=redefined-outer-name
    ...
    optimizer = Ranger(params, lr=c.lr)
    ...
    for epoch in range(0, c.epochs):
        ...
        train_loss, global_step = train(model, criterion, criterion_st,
                                        ...
                                        global_step, epoch, criterion_gst=criterion_gst, optimizer_gst=optimizer_gst)
        val_loss = evaluate(model, criterion, criterion_st, criterion_gst, ap, global_step,
                            epoch)
