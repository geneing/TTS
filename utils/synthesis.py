...
def run_model(model, inputs, CONFIG, truncated, speaker_id=None, style_mel=None, text_gst=False, persistent=False):
    if CONFIG.model == "TacotronGST" and style_mel is not None:
        decoder_output, postnet_output, alignments, stop_tokens = model.inference(
            inputs, style_mel=style_mel, speaker_ids=speaker_id, text_gst=text_gst, persistent=persistent)
    else:
        if truncated:
            ...
            decoder_output, postnet_output, alignments, stop_tokens = model.inference(
                inputs, speaker_ids=speaker_id, text_gst=text_gst, persistent=persistent)
...
def synthesis(model,
              ...
              do_trim_silence=False, text_gst=False, persistent=False):
    ...
    decoder_output, postnet_output, alignments, stop_tokens = run_model(
        model, inputs, CONFIG, truncated, speaker_id, style_mel, text_gst, persistent)
